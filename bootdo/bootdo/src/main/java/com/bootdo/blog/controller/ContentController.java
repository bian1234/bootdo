...
public class ContentController extends BaseController {
	...
	@RequiresPermissions("blog:bContent:add")
	@PostMapping("/save")
	public Rest save(ContentDO bContent) {
		...
		if (count > 0) {
			return Rest.ok().put("cid", bContent.getCid());
		}
		return Rest.error();
	}
	...
	public Rest update(ContentDO bContent) {
		if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
			return Rest.error(1, "演示系统不允许修改,完整体验请部署程序");
		}
		bContent.setGtmCreate(new Date());
	...
	@ResponseBody
	public Rest remove(@RequestParam("ids[]") Long[] cids) {
		if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
			return Rest.error(1, "演示系统不允许修改,完整体验请部署程序");
		}
