...
public class ModelController extends BaseController{
    ...
    public Rest remove(@PathVariable("id") String id) {
    ...
    public Rest deploy(@PathVariable("id") String id) throws Exception {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return Rest.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
        //获取模型
        ...
        if (bytes == null) {
            return Rest.error("模型数据为空，请先设计流程并成功保存，再进行发布。");
        }
        ...
        if (model.getProcesses().size() == 0) {
            return Rest.error("数据模型不符要求，请至少设计一条主线流程。");
        }
