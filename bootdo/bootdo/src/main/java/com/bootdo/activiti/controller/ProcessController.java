...
public class ProcessController extends BaseController{
    ...
    public Rest deploy(String exportDir, String category, MultipartFile file) {
...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
    ...
    public Rest convertToModel(@PathVariable("procDefId") String procDefId, RedirectAttributes redirectAttributes) throws UnsupportedEncodingException, XMLStreamException {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return Rest.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
        ...
        try {
            ...
            return Rest.ok( "转换模型成功，模型ID=" + modelData.getId());
        } catch (Exception e) {
            e.printStackTrace();
            return Rest.ok( "转换模型失败");
        }
    ...
    public Rest batchRemove(@RequestParam("ids[]") String[] ids) {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return Rest.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
        for (String id : ids) {
