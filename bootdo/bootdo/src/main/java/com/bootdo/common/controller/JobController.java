...
public class JobController extends BaseController{
	...
	public Rest info(@PathVariable("id") Long id) {
		...
		return Rest.ok().put("taskScheduleJob", taskScheduleJob);
	}
	...
	@ResponseBody
	@PostMapping("/save")
	public Rest save(TaskDO taskScheduleJob) {
	...
	@ResponseBody
	@PostMapping("/update")
	public Rest update(TaskDO taskScheduleJob) {
		if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
			return Rest.error(1, "演示系统不允许修改,完整体验请部署程序");
		}
	...
	@ResponseBody
	public Rest changeJobStatus(Long id, String cmd ) {
		if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
			return Rest.error(1, "演示系统不允许修改,完整体验请部署程序");
		}
		String label = "停止";
		...
		try {
			...
			return Rest.ok("任务" + label + "成功");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return Rest.ok("任务" + label + "失败");
	}
